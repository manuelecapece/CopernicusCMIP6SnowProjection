function plotDati(snow_basilicata,variable,time,years_string, isInApp, app)
...
a = area(ax,T_interpolato,snow_interp_T,'FaceAlpha', 0.3);
a.FaceColor = rgb('SkyBlue');
a.EdgeColor =  rgb('LightSteelBlue');
a.LineWidth = 0.5;
set (ax, 'Yscale', 'log')
xlabel(ax,testoXlabel, 'FontSize', 13);
ylabel(ax,testoYlabel,'FontSize', 13);
title(ax,testoTitolo,'FontSize', 16);

hold(ax,'on');

%Grafico a barre della quantita di neve in funzione degli anni
bh = bar(ax,years_date,snow_mean_reshaped,'BarWidth', 1,'LineWidth',1);
...
legend(ax,'Media annua','Gennaio', 'Febbraio','Marzo','Aprile','Dicembre','FontSize', 13);
ax.XTick = years_date;
